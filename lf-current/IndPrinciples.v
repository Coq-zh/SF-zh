...
(** 理解了柯里-霍华德同构及其 Coq 实现之后，我们可以深入学习归纳法则。 *)
...
(** 每次我们使用 [Inductive] 来声明数据类型时，Coq 会自动为这个类型生成
    _'归纳法则'_。这个归纳法则也是定理：如果归纳地定义了 [t]，那么对应的
    归纳法则是 [t_ind]。这个自然数的归纳法则： *)
...
(** [induction] 策略是对归纳法则的直接包装，其内核，仅仅是执行 [apply t_ind]。
...
(** Coq 为每一个 [Inductive] 定义的数据类型生成了归纳法则，包括那些非递归的。
    尽管我们不需要使用归纳来证明非递归数据类型的性质，但归纳法则的概念仍然适用于它们：
...
(** **** 练习：1 星, standard, optional (rgb)  
    ...
    请写出对这个数据类型 Coq 将会生成的归纳法则。在纸上或注释中写下你的答案，
    然后同 Coq 打印的结果比较。 *)
...
(** **** 练习：1 星, standard, optional (byntree_ind)  
    ...
    请写出对这个数据类型 Coq 将会生成的归纳法则。（与之前一样，在纸上或注释中写下你的答案，
    然后同 Coq 打印的结果比较。） *)
...
(** 接下来，多态数据结构会是怎样呢？
    ...
    请注意_'全部的'_归纳法则都被 [X] 所参数化。也即，[list_ind] 可认为是一个
    多态函数，当被应用类型 [X] 时，返回特化在类型 [list X] 上的归纳法则。 *)

(** **** 练习：1 星, standard, optional (tree)  

    请写出对这个数据类型 Coq 将会生成的归纳法则。同 Coq 打印的结果比较你的答案。*)
...
(** 之前，我们仔细学习了 Coq 为归纳定义的_'集合'_生成的归纳法则。 像 [even]
    ...
    _'数字'_的陈述：因此，我们想要让归纳法则允许通过对证据进行归纳来
    证明关于数字的性质。
    ...
    ev_ind_max : forall P : (forall n : nat, even n -> Prop),
     ...
     - 因为 [even] 被数字 [n] 所索引（任何 [even] 的对象 [E] 都是某个数字 [n]
       ...
       归纳法则涉同时及到数字和这个数字是偶数的证据。

     - 由于有两种方法来给出偶数性质的证据（因为 [even] 有两个构造子），应用归纳法则生成
       了两个子目标：
