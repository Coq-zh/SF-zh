...
(** Coq 为每一个 [Inductive] 定义的数据类型生成了归纳原理，包括那些非递归的。
    尽管我们不需要使用归纳来证明非递归数据类型的性质，归纳原理的概念仍然适用于它们：
...
(** 接下来，多态数据结构会是怎样呢？
    ...
    请注意_'全部的'_归纳原理由 [X] 所参数化。也即，[list_ind] 可被认为是一个
...
(** 第二个是更好的，尽管它看起来不那么对称了。为什么呢？因为它会生成更简单的归纳原理。 *)
...
(** 问：命题 [P] 的形式化证明和同一个命题 [P] 的非形式化证明之间是什么关系？
  ...
  一种中庸之道是——一份包括了必要的洞察的证明（免去读者像我们一开始一样辛苦地寻找证明）
  配合模式化部分的高层次建议来节省读者自己重新构造这些东西的时间（比如，归纳假设是什么，
  以及归纳证明中每个情形的证明责任），但没有过多的细节以至于主要的概念和想法受到干扰。

  我们在本章中已经仔细查看了形式化的归纳证明的「底层原理」，现在是时候来看看非形式化的归纳证明了。

  在现实世界的数学交流中，证明的书写既有冗长的，也有非常简洁的。
  尽管理想状态是二者中间的某种形式，但从有一点冗长的证明开始学习是有好处的。
  同时，在学习的过程中，有一个明确的标准来进行比较也是有益的。以此为目的，
