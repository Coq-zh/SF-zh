...
(** 首先，我们定义一小段 Ltac 宏，将常用的模式压缩成单个命令。 *)
...
(** 迄今为止，我们的证明脚本大多是按名称来应用相关的假设或引理的，一次一个。 *)
...
(** 在搜索当前目标的潜在证明时， [auto] 会同时考虑当前上下文中的前提，
    ...
    某些关于相等性和逻辑运算符的事实默认已经安装到提示数据库中了。 *)
...
(** 如果在证明中我们会反复用到某个策略，呢么可以使用一个 [Proof]
    命令的变体将它作为证明中的默认策略。例如 [Proof with t]（其中 [t]
