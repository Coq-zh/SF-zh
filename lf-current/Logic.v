...
(**
    在前面的章节中，我们已经见过很多对事实的宣言（即_'命题'_）
    以及如何用证据展示其正确性（即_'证明'_）的例子了。特别是，
    我们证明了大量形如 [e1 = e2] 的_'相等性命题'_、形如 [P -> Q]
    的蕴含式、以及形如 [forall x, P x] 的量化命题。

    在深入细节之前，我们先来探讨一下 Coq 中数学表达式的地位。
    回忆一下，Coq 是一门拥有_'类型'_语言，也就是说，一切有意义的表达式都具有类型。
    逻辑性的宣言也不例外，我们试图在 Coq 中证明的一切语句都有名为 [Prop]
    的类型，即_'命题类型'_。我们可以用 [Check] 命令来查看：  *)
...
(** 我们也可以写出_'参数化'_的命题 -- 也就是说，函数接受某些类型的参数，
    然后返回一个命题。 *)
...
(** 由于按照假设对某个目标应用定理会产生与该定理的假设一样多的子目标。
...
(** 以上就是证明合取语句的方法。要反过来使用，即_'使用'_合取假设来帮助证明时，
    我们会采用 [destruct] 策略。

    如果当前证明上下文中存在形如 [A /\ B] 的假设 [H]，那么
    [destruct H as [HA HB]] 将会从上下文中移除 [H] 并增加 [HA] 和 [HB]
    两个新的假设，前者宣言 [A] 为真，而后者宣言 [B] 为真。 *)
...
(** 为什么我们要麻烦地将 [n = 0] 和 [m = 0] 这两个假设放一条合取语句中呢？
    完全可以用两条独立的假设来陈述此定理啊： *)
...
(** 就此定理而言，两种方式都可以。不过理解如何证明合取假设非常重要，
...
(** 这样的不等性语句频繁出现，足以让我们为其定义一种特殊的记法 [x <> y]： *)
...
(** 为了习惯用 Coq 处理否定命题，我们需要一些练习。
    即便你十分清楚为什么某个否定命题为真，但在初次设置正确以便让 Coq
    理解它时也会有点棘手。以下常见事实的证明留给你热身。 *)
...
(** 现在我们可以用这些事实配合 [rewrite] 与 [reflexivity]
    对涉及等价性的陈述给出流畅的证明了。以下是之前 [mult_0]
    包含三个变量的版本： *)
...
(** [apply] 策略也可以与 [<->] 一同使用。当给定一个等价性命题作为
    [apply] 的参数时，它会试图猜出应该使用等价性命题的哪一边。 *)
...
(** 反之，如果我们的的上下文中有形如 [exists x, P] 的存在假设，
    可以将其解构得到一个例证 [x] 和一个陈述 [P] 对于 [x] 成立的假设。 *)
...
(** 我们学过的逻辑联结词为我们提供了丰富的用简单命题构造复杂命题的词汇。
    为了说明，我们来看一下如何表达「元素 [x] 出现在列表 [l] 中」这一宣言。
    注意此性质有着简单的递归结构： *)
...
(** 从操作上来说，这种类比可以更进一步：由于定理可以作为函数应用到对应类型的假设上，
    我们就可以专注于结论而不必使用中间断言了。例如，假设我们想要证明以下结论： *)
...
(** 一种更优雅的方式是直接将 [plus_comm] 应用到我们想要以至实例化的参数上，
...
(** Coq 的逻辑核心，即_'归纳构造演算（Calculus of Inductive Constructions）'_系统，
    ...
    例如，在主流的纸笔数学家中使用最普遍的「策梅洛-弗兰克尔集合论（ZFC）」中，
    ...
    除非我们引入新的公理来丰富其逻辑核心。作为本章的结束，
    我们将探讨这两个世界之间最显著的区别。 *)

(* ================================================================= *)
(** ** 函数的外延性 *)

(** 目前为止我们所看见的相等性断言基本上都只考虑了归纳类型的元素
    （如 [nat]、[bool] 等等）。然而由于 Coq 的相等性运算符是多态的，
...
(** 然而，我们可以用 [Axiom] 命令将函数的外延性添加到 Coq 的核心逻辑系统中。 *)
...
(** 我们可以用 [Print Assumptions] 命令查看某个证明依赖的所有附加公理。 *)
...
(** 然而，即便布尔值和某个宣言的命题式在逻辑上是等价的，但它们在_'操作上'_
    并不一样。
...
(** Coq 会抱怨 [n = 2] 的类型是 [Prop]，而它想要一个 [bool]
    ...
    中的项进行计算可以实现一些自动推理，这种技术被称为_'反射证明（Proof
...
(** 通用的排中律默认在 Coq 中并不可用，这一点或许很奇怪，毕竟，
    任何给定的宣言都是非真即假的。尽管如此，不假设排中律的成立仍有其有限：
    Coq 中的陈述可以构造出比标准数学中同样陈述更强的宣言。特别是，
