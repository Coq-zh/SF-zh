...
(** 注意，和归纳自然数时一样，此处 [induction] 策略的 [as...] 从句为在
    「[l1] 由构造子 [cons] 构造而来」这一情况时出现的「更小的列表」和归纳前提取了名字。
...
(** 我们已经见过很多需要使用之前证明过的结论（例如通过 [rewrite]）来证明的定理了。
    ...
    Coq 的 [Search] 命令在这时就非常有用了。执行 [Search foo] 会让 Coq
...
Fixpoint nth_error' (l:natlist) (n:nat) : natoption :=
 ...
 (** Coq 的条件语句和其它语言中的一样，不过加上了一点更为一般化的特性。
    ...
    归纳定义的类型上使用条件表达式。当守卫式（guard）求值为 [Inductive]
