...
(** 本电子书是一系列关于 _'《软件基础》'_ 的课程，它阐明了可靠软件背后的数学根基。
    书中的主题包括基本的逻辑概念、计算机辅助定理证明、Coq 证明助理（Proof Assistant）、
    函数式编程（Functional Programming）、操作语义（Operational Semantic）、
    用于论证软件的逻辑、以及静态类型系统（Static Type System）。
    本课程的目标受众包括从高年级本科生到博士以及研究者在内的广大读者。
    本书并未假定读者有逻辑学或编程语言的背景，不过一定的数学熟练度会很有帮助。

    本课程的最大创新之处在于，所有内容都百分之百地形式化且通过了机器验证，
    也就是说，所有的文字都是字面意义上的 Coq 脚本。它旨在配合 Coq
    交互式会话一起阅读。书中的所有细节都完全用 Coq 形式化了，习题也被设计成用
    Coq 来解决。

    每卷书的文件都被精心地组织，核心章节作为主线贯穿始终，涵盖了一学期的内容；
    「支线」中则包含额外的主题。所有的核心章节都适合高年级本科生与研究生。

    本书为第一卷 _'逻辑基础'_，它向读者介绍了函数式编程的基本概念、构造逻辑以及
    Coq 证明助理，为其它卷本奠定了基础。 *)
...
(** 构建可靠的软件非常困难。现代系统的规模，复杂度，参与构建过程的人数，
    ...
    更不用说百分之百地正确了。同时，由于信息处理继续渗透到社会的各个方面，
    程序错误和漏洞的代价变得越来越高昂。

    为了应对这些挑战，计算机科学家和软件工程师发展了一系列提升软件质量的方法，
    从为管理软件项目的团队提供建议（如极限编程，Extreme Programming），
    到库的设计原理（如模型-视图-控制器，Model-View-Controller；发布-订阅模式，
    Publish-Subscribe）以及编程语言（面向对象编程，Object Oriented Programmin；
    面向剖面编程，Aspect Oriented Programming；函数式编程，Functional Programming）
    的设计哲学，还有用于指定和论证软件性质的数学技术，以及验证这些性质的工具。
    _'《软件基础》'_ 系列着重于最后一种方法。

    本书将以下三条概念的线索穿插在一起：

    （1）_'逻辑学'_ 中的基本工具，用于准确地提出并论证关于程序的假设；

    （2）_'证明助理'_ 用于构造严谨的逻辑论据；

    （3）_'函数式编程'_ 思想，同时作为编程方法简化程序的论证，
         以及架起程序与逻辑学之间的桥梁。

    扩展阅读的建议可在 [Postscript] 一章中找到；
    所有引述过的著作的书目信息可以在 [Bib] 一章中找到。 *)

(* ================================================================= *)
(** ** 逻辑学 *)

(** 逻辑学的领域是研究 _'证明'_，即对特定命题的真伪性进行不容置疑的论证。
    有关逻辑学在计算机科学中核心作用的书卷汗牛充栋。Manna 和 Waldinger
    称之为「计算机科学的微积分」，而 Halpern 的论文
    _'On the Unusual Effectiveness of Logic in Computer Science'_ 中则收录了
    大量逻辑学为计算机科学提供的洞察力和至关重要的工具。的确，他们发现：
    「事实上，逻辑学在计算机科学中远比在数学中更加有效。
    这相当引人注目，特别是由于过去一百年来，逻辑学发展的动力大都来自于数学。」

    特别来说，_'归纳证明'_ 的基本概念在计算机科学中无处不在。
...
(** 逻辑学与计算机科学之间的思想交流并不是单方面的，
    计算机科学也对逻辑学做出了重要的贡献，
    其中之一就是发展了帮助构造逻辑命题的证明的软件工具。
    这些工具分为两大类：

       - _'自动定理证明器'_ 提供了一键式操作：它们接受一个命题，
         然后返回 _'真'_ 或 _'假'_（有时为 _'未知：超时'_ ）。
         尽管它们的能力仅限于特定种类的推理中，但在近几年却快速成熟，
         并应用于多种场合，此类工具包括 SAT 求解器，SMT 求解器以及模型检查器
         （Model Checker）。

       - _'证明助理'_ 是一种混合式工具，它能将构建证明中比较常规的部分自动化，
         而更困难的部分则依赖于人类解决。广泛使用证明助理包括
    ...
    系统的内核是一个简单的证明检查器，它保证只会进行正确的推论步骤。
    在此内核之上，Coq 环境提供了高级的证明开发设施，包括一个庞大的库，
    其中包含各种定义和引理；强大策略，用于半自动化构造证明；
    还有一个专用的编程语言，用于为特殊情况定义新的自动证明策略。
...
(** _'函数式编程'_ 不仅表示可以在几乎任何编程语言中使用的各种习语（Idiom），
    ...
    执行代码的唯一作用应当是只产生一个结果：计算应当没有有 _'副作用'_，
    即与输入/输出、可变量的赋值、指针重定向等分离。例如，_'命令式'_
...
(** Coq 可以在 Windows、Linux 和 macOS 上运行。我们需要：
       ...
       - 安装最近版本的 Coq，它可以从 Coq 主页获得。本书中的文件均已通过了
         Coq 8.7.1 的测试。

       - 一个能跟 Coq 交互的 IDE。目前为止有两种选择：
           ...
           - CoqIDE 是个更加简单的独立 IDE。它随 Coq 一起发布，
             所以如果你安装了 Coq，它就应该能用。你也可以从头编译安装它，
             不过在某些平台上还需要额外安装 GUI 库之类的包。 *)
...
(** 每一章都包含大量的习题。每个习题都有标有「星级」，其含义是：
    ...
    只做非进阶、非可选的习题已经能不错地掌握核心概念了。
    可选习题会提供一些关键概念的额外练习，以及一些读者可能会感兴趣的次级主题。
    进阶练习留给想要更多挑战和更深理解的读者。
...
(** 如果您有意用这些课件授课，那肯定会发现希望改进、提高或增加的材料。
    ...
    我们为您设置 subversion 源码库和开发者邮件列表的访问权限。
